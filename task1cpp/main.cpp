...
void saveArrayToFile(ofstream& file) {
    for (const auto& pair : arrays) {
        ...
        for (int i = 0; i < pair.second->getSize(); i++) {
            string value = arrayGetElementIndex(pair.second, i);
            file << value;
...
void saveTreeToFile(ofstream& file) {
    for (const auto& pair : trees) {
        ...
        if (pair.second->root != pair.second->nil) {
            queue<TNode*> que;
            que.push(pair.second->root);
            while (!que.empty()) {
                TNode* current = que.front();
                que.pop();
                if (current != pair.second->nil) {
                    file << current->key << " ";
                    if (current->left != pair.second->nil) {
                        que.push(current->left);
                    }
                    if (current->right != pair.second->nil) {
...
void saveHashChainToFile(ofstream& file) {
    for (const auto& pair : hashChains) {
        ...
        for (int i = 0; i < pair.second->capacity; i++) {
            HNode* current = pair.second->table[i];
            while (current != nullptr) {
                if (!firstElement) {
                    file << " ";
                }
...
void saveHashAddressToFile(ofstream& file) {
    for (const auto& pair : hashAddresses) {
        ...
        bool firstElement = true;
        for (int i = 0; i < pair.second->capacity; i++) {
            if (!pair.second->table[i].isEmpty && !pair.second->table[i].isDeleted) {
                if (!firstElement) {
                    file << " ";
                }
                file << pair.second->table[i].key << ":" << pair.second->table[i].data;
                firstElement = false;
            }
        }
        file << "\n";
