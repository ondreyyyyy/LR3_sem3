...
TEST(TreeTests, DuplicateInsertIgnored) {
    ...
    tree.insert(8);
    int count = 0;
    if (tree.findNode(8) != tree.getNil()) count++;
    EXPECT_EQ(count, 1);
}
...
TEST(TreeTests, FindExistingAndMissing) {
    Tree tree;
    tree.insert(4);
    EXPECT_NE(tree.findNode(4), tree.getNil());
    EXPECT_EQ(tree.findNode(99), tree.getNil());
}
...
TEST(TreeTests, FindMinMax) {
    ...
    tree.insert(2);
    tree.insert(15);
    auto minNode = tree.findMinimum(tree.getRoot());
    auto maxNode = tree.findMaximum(tree.getRoot());
    EXPECT_EQ(minNode->getKey(), 2);
    EXPECT_EQ(maxNode->getKey(), 15);
}
...
TEST(TreeTests, DeleteSingleNode) {
    ...
    tree.insert(10);
    auto node = tree.findNode(10);
    ASSERT_NE(node, tree.getNil());
    tree.deleteNode(node);
    EXPECT_EQ(tree.findNode(10), tree.getNil());
}

TEST(TreeTests, DeleteNilThrows) {
    Tree tree;
    EXPECT_THROW(tree.deleteNode(tree.getNil()), std::runtime_error);
}
...
TEST(TreeTests, RotateCheck) {
    ...
    for (int i = 0; i < 30; i++) {
        EXPECT_NE(tree.findNode(i), tree.getNil());
    }
    ...
    EXPECT_EQ(tree.findNode(16), tree.getNil());
    EXPECT_EQ(tree.findNode(4), tree.getNil());
    EXPECT_EQ(tree.findNode(5), tree.getNil());
    EXPECT_EQ(tree.findNode(14), tree.getNil());
    EXPECT_EQ(tree.findNode(25), tree.getNil());
    EXPECT_EQ(tree.findNode(13), tree.getNil());
    EXPECT_EQ(tree.findNode(20), tree.getNil());
    EXPECT_EQ(tree.findNode(3), tree.getNil());
    EXPECT_EQ(tree.findNode(27), tree.getNil());
}

TEST(TreeTests, GetRootAndNil) {
    Tree tree;
    EXPECT_NE(tree.getNil(), nullptr);
    EXPECT_EQ(tree.getRoot(), tree.getNil());
}

TEST(TreeTests, GetKeyColorAndPointers) {
    TNode* parent = nullptr;
    TNode node(42, RED, parent);
    EXPECT_EQ(node.getKey(), 42);
    EXPECT_EQ(node.getColor(), RED);
