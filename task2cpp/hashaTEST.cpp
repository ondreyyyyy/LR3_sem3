...
TEST(hashaTest, deleteFromTable) {
    HashA table;
    table.addElementA("ww1", 77);
    table.addElementA("ww2", 666);
...
TEST(hashaTest, createNode) {
    HashA table;
    HANode* node = new HANode("ww1", 77);
    table.addElementA(node->getKey(), node->getData());
...
TEST(hashaTest, GetTableAndCapacity) {
    HashA hash;
    EXPECT_NE(hash.getTable(), nullptr);
    EXPECT_GT(hash.getCapacity(), 0);
}

TEST(hashaTest, GetKeyDataIsDeletedAndIsEmpty) {
    HANode node("key", 123);
    EXPECT_EQ(node.getKey(), "key");
    EXPECT_EQ(node.getData(), 123);
    EXPECT_FALSE(node.getIsDeleted());
    EXPECT_FALSE(node.getIsEmpty());
}